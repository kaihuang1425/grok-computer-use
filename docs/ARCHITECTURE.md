...
Grok Computer Use separates planning, execution, and verification.
...
- `.grok/config.toml` — project-scoped local MCP registration using the repo `.venv`.
- `src/grok_cua/server.py` — MCP surface.
- `src/grok_cua/adapters/windows.py` — Windows UIA, screenshots, keyboard/mouse fallback.
- `src/grok_cua/core/router.py` — execution-surface scoring and fallback ordering primitives.
...
- Route scoring exists as a core primitive but is not yet a fully autonomous multi-surface router.
